...
export default function LayerStylePanel({ layers, styles, onLocalChange }: Props) {
  ...
  return (
    <div className="mt-4">
      ...
      {open && (
        <div className="mt-4 bg-white rounded-xl shadow-md p-4 sm:p-6 space-y-6">
          {layers.map((layer) => {
            const st = styles[layer.id] || {}
            return (
              <div key={layer.id} className="border-b border-gray-200 pb-4 last:border-0 last:pb-0">
                <h4 className="font-semibold text-gray-700 mb-2 text-sm sm:text-base">{layer.name}</h4>
                ...
                <label className="block mb-2 text-xs sm:text-sm">
                  ...
                  <input
                    ...
                    className="w-full"
                  />
                  <span className="text-xs sm:text-sm ml-2">{Math.round((st.opacity ?? 1) * 100)}%</span>
                </label>
                ...
                <label className="block mb-2 text-xs sm:text-sm">
                  ...
                  <input
                    ...
                    className="w-full"
                  />
                  <span className="text-xs sm:text-sm ml-2">{Math.round((st.fillOpacity ?? 0.5) * 100)}%</span>
                </label>
                ...
                <label className="block mb-2 text-xs sm:text-sm">
                  ...
                  <input
                    ...
                    className="w-full"
                  />
                  <span className="text-xs sm:text-sm ml-2">{st.radius ?? 5}px</span>
                </label>
              </div>
            )
          })}
